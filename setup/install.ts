...
async function main() {
  ...
  console.log("");
  console.log(bold("  Beckett — Setup"));
  ...
  // Prompt for USER_TIMEZONE if not already set
  const envPath = join(PROJECT_ROOT, ".env");
  let envContent = "";
  try {
  ...
  if (!envContent.includes("USER_TIMEZONE=") || envContent.includes("USER_TIMEZONE=UTC")) {
    console.log("");
    process.stdout.write(`  What is your timezone? ${dim("(e.g., America/New_York)")} [UTC]: `);
    const tzLine = await new Promise<string>((resolve) => {
      let data = "";
    ...
    const tz = tzLine || "UTC";
    if (envContent.includes("USER_TIMEZONE=")) {
      envContent = envContent.replace(/USER_TIMEZONE=.*/, `USER_TIMEZONE=${tz}`);
    } else {
      envContent += `\nUSER_TIMEZONE=${tz}\n`;
    }
    await Bun.write(envPath, envContent);
    console.log(`  ${PASS} Timezone set to ${tz}`);
  }

  // Prompt for ASSISTANT_NAME if not already set
  if (!envContent.includes("ASSISTANT_NAME=") || envContent.includes("ASSISTANT_NAME=Beckett")) {
    console.log("");
    process.stdout.write(`  What would you like to name your assistant? ${dim("(default: Beckett)")} [Beckett]: `);
    const nameLine = await new Promise<string>((resolve) => {
      let data = "";
      process.stdin.setEncoding("utf-8");
      process.stdin.once("data", (chunk: string) => {
        data = chunk.trim();
        resolve(data || "Beckett");
      });
    });
    const assistantName = nameLine || "Beckett";
    // Re-read in case timezone prompt modified it
    envContent = await Bun.file(envPath).text();
    if (envContent.includes("ASSISTANT_NAME=")) {
      envContent = envContent.replace(/ASSISTANT_NAME=.*/, `ASSISTANT_NAME=${assistantName}`);
    } else {
      envContent += `\nASSISTANT_NAME=${assistantName}\n`;
    }
    await Bun.write(envPath, envContent);
    console.log(`  ${PASS} Assistant name set to ${assistantName}`);
  }
